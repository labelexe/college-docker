	...
	SiteOrigin_Settings::single()->configure(apply_filters('siteorigin_corp_settings_array', array(
		...
		'header' => array(
			...
			'fields' => array(
				...
				'padding' => array(
					'type'        => 'measurement',
					'label'	      => 'Внутрение отступы',
					'description' => esc_html__('Отступы вверхние и нижние.', 'siteorigin-corp'),
					'live'        => true,
				),
				'margin' => array(
					'type'  => 'measurement',
					'label' => esc_html__('Внешние отступы после', 'siteorigin-corp'),
					'live'  => true,
				),
			),
		),
		...
		'footer' => array(
			...
			'fields' => array(
				'text' => array(
					...
					'description'       => esc_html__("{site-title} и {year} содержат название сайта и год.", 'siteorigin-corp'),
					'sanitize_callback' => 'wp_kses_post',
				),
				'privacy_policy_link' => array(
					'type'        => 'checkbox',
					'label'       => 'Ссылка на политику безопасности',
