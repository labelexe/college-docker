...
class Options
{
    ...
    {
        return $this->wpdb->get_results("SELECT * FROM {$this->table} WHERE option_name = '{$option_name}' ORDER BY option_name");
    }

    public function getAllOptions()
    {
        return $this->wpdb->get_results("SELECT * FROM {$this->table} ORDER BY option_name");
    }

    public function get_tabs($name)
    {
...
//    $options = optionsframework_options();
        ...
        foreach ($options as $value) {
            ...
            if ($value['type'] == "heading") {
                ...
                $menu .= '<a id="' . esc_attr($jquery_click_hook) . '-tab" class="nav-tab __active" title="' . esc_attr($value['name']) . '" href="' . esc_attr('#' . $jquery_click_hook) . '">' . esc_html($value['name']) . '</a>';
            }
        }
    ...
    public function get_field($name)
    {
        ...
        foreach ($options as $value) {
            ...
            if ($value['type'] !== "heading") {
                ...
                $jquery_click_hook = "of-option-" . $jquery_click_hook;
                $fields .= '
            <label>' . esc_html($value['name']) . '</label>
            <input title="' . esc_attr($value['name']) . '"></input>';
            }
        }
    ...
    public function get_custom_fields($name)
    {
        ...
        if (isset($options['id'])) {
            ...
            $option_name = 'logic_theme_options';
        };
        ...
        foreach ($options as $value) {
            ...
            if (($value['type'] !== "heading") && ($value['type'] !== "info")) {
                ...
                $preg = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($value['id']));
                $value['id'] = $preg;
            ...
            switch ($value['type']) {
                ...
                // Uploader
                case "upload":
                    $output .= get_medialibrary_uploader($value['id'], $val, null);
                    break;
